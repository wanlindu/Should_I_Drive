...
def query_for_compute_occupancy(lat,lon, distance, day, ti):
    ...
    postgreSQL_query_for_occupied = """SELECT * FROM processed_events_""" + day +""" WHERE ST_DWithin(where_is,ST_GeomFromText(""" + location +""", 4326),"""+ distance + """) AND processed_events_""" + day +"""."StartWindow" IN """ + start_window_con + """ AND processed_events_""" + day +"""."EndWindow" in """ + end_window_con + """;"""
    print(postgreSQL_query_for_occupied)
    cursor_2.execute(postgreSQL_query_for_occupied)
    t1 = datetime.datetime.now().timestamp()
    print("from database")
    print(t1 - t0)
    occupied_df = pd.DataFrame(cursor_2.fetchall(), columns = ['POST_ID','EventDate','StartTime','EndTime','Day_Of_Week','StartWindow','EndWindow','CAP_COLOR','longitude','latitude','where_is'])
    print(occupied_df)
    #Find unique POST_IDs
    pre = occupied_df.groupby(['EventDate', 'POST_ID']).count()
    res = pre.groupby(['EventDate']).count().mean()
    return res[0]
...
def map(address, distance, day, time):
    ...
    occupied = query_for_compute_occupancy(lat,lon, distance, day, time)
    if(occupied != None):
    ...
    fig = px.scatter_mapbox(total_meter_locations, lat="LATITUDE", lon="LONGITUDE", zoom=16, height=450)
